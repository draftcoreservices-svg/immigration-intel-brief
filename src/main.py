...
def relevance_score(text: str, keywords: List[str], high_signal: List[str], source: str) -> int:
    """Simple, conservative relevance scoring.

    We score on:
    - keyword matches (broad)
    - high-signal phrase matches (strong indicators)
    - source weighting (official sources)
    - critical triggers
    """
    t = (text or "").lower()
    kws = [k.lower() for k in (keywords or [])]
    hs = [h.lower() for h in (high_signal or [])]
    ...
    if "gov.uk" in src:
        score += 2
    if "legislation" in src:
        score += 2
    if "judiciary" in src or "utiac" in src or "tribunal" in src:
        score += 3
    if "parliament" in src:
        score += 1

    # Critical triggers are strong relevance signals
    score += min(8, critical_score(t))
    return score
...
def filter_items(
    ...
    """
    kws = [k.lower() for k in (keywords or [])]
    hs = [h.lower() for h in (high_signal_phrases or [])]
    ex = [e.lower() for e in (exclude_keywords or [])]
    ...
    for it in items:
        ...
        # Hard exclusions (unless core override)
        if not core_override and any(e in hay for e in ex):
            continue

        rscore = relevance_score(hay, keywords=kws, high_signal=hs, source=it.get("source", ""))
        if not core_override and rscore < int(min_score):
            continue

        it["__critical_score"] = critical_score(hay)
        it["__relevance_score"] = int(rscore)

        # Coarse priority bucketing
        section = classify_section(it.get("source", ""))
        if section == "Courts & Tribunals":
            it["__priority"] = "HIGH"
        elif section in ("GOV.UK", "Legislation") and int(it["__critical_score"]) >= 4:
            it["__priority"] = "HIGH"
        elif section == "Parliament":
            it["__priority"] = "WATCH"
        elif int(it["__relevance_score"]) >= (int(min_score) + 4):
            it["__priority"] = "WATCH"
        else:
            it["__priority"] = "BACKGROUND"

        out.append(it)

    out.sort(
        key=lambda x: (
            0 if x.get("__priority") == "HIGH" else 1 if x.get("__priority") == "WATCH" else 2,
            -int(x.get("__critical_score", 0)),
            -int(x.get("__relevance_score", 0)),
        )
    )
    return out
...
def render_email_html(
    ...
    combined = [
        (
            0 if it.get("__priority") == "HIGH" else 1 if it.get("__priority") == "WATCH" else 2,
            -int(it.get("__critical_score", 0)),
            -int(it.get("__relevance_score", 0)),
            it,
        )
        for it in (updated_items + new_items)
    ]
    combined.sort(key=lambda x: (x[0], x[1], x[2]))
    top3 = [t[3] for t in combined[:3]]

    critical_items = [it for it in (updated_items + new_items) if it.get("__priority") == "HIGH"][:8]
    ...
    def card(it: Dict[str, Any], badge: str, prev: Optional[str] = None) -> str:
        ...
        if pub:
            meta_bits.append(html.escape(str(pub)[:40]))
        meta = " • ".join(meta_bits)
        meta_html = (
            f"<div style='color:#64748B;font-size:12px;margin-top:2px;'>{meta}</div>" if meta else ""
        )
...
"""
    ...
    if critical_items:
        lis = []
        for it in critical_items[:6]:
            title = html.escape(it.get("title", "(untitled)"))
            url = html.escape(it.get("url", ""))
            badge = "UPDATED" if it in updated_items else "NEW"
            lis.append(
                f"<li style='margin:0 0 6px 0;'>"
                f"<span style='font-size:11px;font-weight:900;padding:2px 8px;border-radius:999px;border:1px solid #E5E7EB;background:#FFFFFF;margin-right:6px;color:#0F172A;'>{badge}</span>"
                f"<a href='{url}' style='color:#0F172A;text-decoration:none;font-weight:800;'>{title}</a>"
                f"</li>"
            )
        parts.append(
            "<div style='margin:14px 0 16px 0;padding:14px;border:1px solid #E5E7EB;border-radius:14px;background:#FFFFFF;'>"
            "<div style='font-weight:900;font-size:14px;color:#0F172A;margin:0 0 8px 0;'>High impact</div>"
    ...
    def render_priority_block(title: str, items: List[Dict[str, Any]], badge_for: str) -> None:
        if not items:
            return
        parts.append("<div style='margin:18px 0 10px 0;border-top:1px solid #E5E7EB;'></div>")
        parts.append(f"<h2 style='margin:14px 0 10px 0;font-size:16px;color:#0F172A;'>{html.escape(title)}</h2>")
        by_section: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
        for it in items:
            by_section[classify_section(it.get("source", ""))].append(it)
        for section in ["GOV.UK", "Legislation", "Parliament", "Courts & Tribunals", "Other"]:
            if not by_section.get(section):
                continue
            parts.append(
                f"<h3 style='margin:14px 0 8px 0;font-size:14px;color:#0F172A;'>{html.escape(section)}</h3>"
            )
            for it in by_section[section][:8]:
                if badge_for == "UPDATED":
                    parts.append(card(it, "UPDATED", prev=it.get("previously_covered")))
                else:
                    parts.append(card(it, "NEW"))

    # Prioritise by bucket first, then new/updated
    high_new = [it for it in new_items if it.get("__priority") == "HIGH"]
    high_upd = [it for it in updated_items if it.get("__priority") == "HIGH"]
    watch_new = [it for it in new_items if it.get("__priority") == "WATCH"]
    watch_upd = [it for it in updated_items if it.get("__priority") == "WATCH"]
    back_new = [it for it in new_items if it.get("__priority") == "BACKGROUND"]
    back_upd = [it for it in updated_items if it.get("__priority") == "BACKGROUND"]

    render_priority_block("High impact — updates", high_upd, "UPDATED")
    render_priority_block("High impact — new", high_new, "NEW")
    render_priority_block("Watchlist — updates", watch_upd, "UPDATED")
    render_priority_block("Watchlist — new", watch_new, "NEW")
    render_priority_block("Background — updates", back_upd, "UPDATED")
    render_priority_block("Background — new", back_new, "NEW")
...
def main() -> None:
    ...
    filtered = filter_items(
        ...
        min_score=int(settings.get("relevance_min_score", 6)),
    )
