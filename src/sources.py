...
def fetch_parliament_rss() -> List[Dict[str, Any]]:
    # Keep sources conservative and official.
    feeds = [
        ...
        "https://www.parliament.uk/business/publications/commons/weekly-information-bulletin/rss/",
    ]
    out: List[Dict[str, Any]] = []
    for f in feeds:
...
def fetch_legislation_atom() -> List[Dict[str, Any]]:
    # Default to "new legislation" feed (broader) unless overridden in settings.
    feed = "https://www.legislation.gov.uk/new/data.feed"
    out: List[Dict[str, Any]] = []
...
def fetch_all_sources(settings: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        parl = settings.get("parliament", {}) or {}
        leg = settings.get("legislation", {}) or {}
        jud = settings.get("judiciary", {}) or {}

        items.extend(fetch_parliament_rss())

        if leg.get("atom_new_legislation"):
            # override default feed by temporarily patching
            feed_url = str(leg.get("atom_new_legislation"))
        ...
        if jud.get("rss_utiac"):
            feed_url = str(jud.get("rss_utiac"))
            d = feedparser.parse(feed_url)
            for e in d.entries[:40]:
                items.append(
                    {
        ...
        # Safe fallback: include the defaults
        items.extend(fetch_parliament_rss())
        items.extend(fetch_legislation_atom())
        items.extend(fetch_utiac_rss())
    return items
